...
class Conv1D2(nn.Module):
    def __init__(self, c1,shortcut=True):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        self.cv1=nn.Conv1d(in_channels=c1, out_channels=c1, kernel_size=1)
        #self.cv2 = nn.Conv1d(in_channels=c1, out_channels=c1, kernel_size=1)
    def forward(self, x):   ###x:b*c_i*h*w
        #b,c,w,h=x.shape
        #print('x', x.shape)
        y1_ = x.view([x.size()[0], x.size()[1], -1]) # [B,C,H*w]
        #print('y1_', y1_.shape)
        x2 = self.cv1(y1_)  #b*c_i * h*w   [B,C,H*w]
        #print('x2_', x2.shape)
        #x2 = self.cv2(x2)
        #b=self.cv2(F.normalize(x).view([x.size()[0], x.size()[1], -1]))  # [B,C,H*w]
        #b_T = b.permute([0, 2, 1]) # [B,H*w,C]
        return x2 #torch.matmul(y1_,self.softmax(torch.matmul(b_T,x2))) #[B,H*w,H*w]
...
class SKConv(nn.Module):
    def __init__(self,in_channels,out_channels,stride=1,M=4,r=16,L=32):
        ...
        #self.A =nn.Conv2d(out_channels*M,out_channels*M,1,1,bias=False)
        self.fc2=nn.Conv2d(d,out_channels*M,1,1,bias=False)  # 升维
        #self.se=(out_channels)
    ...
    def forward(self, input):
        ...
        output=[]
        #the part of split
        #print('0',input.size()) #[1, 128, 32, 32]
        ...
        #print('U',U.size())  ### [1, 128, 32, 32]
        s=self.global_pool(U)     # [batch_size,channel,1,1]
        #print('s',s.size()) #([1, 128, 1, 1])
        z=self.fc1(s)  # S->Z降维   # [batch_size,d,1,1] [1, 32, 1, 1])
        #print('z',z.size())
        a_b=self.relu(self.fc2(z)) # Z->a，b 升维 [1, 512, 1, 1]) 论文使用conv 1x1表示全连接。结果中前一半通道值为a,后一半为b   [batch_size,out_channels*M,1,1]
        #print('zab',a_b.size())
        #a_b=self.A(a_b)
        #print('a_b',a_b.shape)
        a_b=a_b.reshape(batch_size,self.M,self.out_channels,-1) #调整形状，变为 两个全连接层的值[batch_size,M,out_channels,1] ([1, 4, 128, 1])
        #print('111',a_b.size())
...
class Bottleneck_(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        #self.cv2 = Conv(c_, c2, 3, 1, g=g)  #SKConv(planes,planes,stride)
        self.cv2 = SKConv(c_, c2,1)
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self,c1, k=1, s=1, p=None):
        ...
        self.fc1 = nn.Linear(32 *5* 5, 128) #input/8 4 4 (640*640)   2*2  (200*200)
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck_c2(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
