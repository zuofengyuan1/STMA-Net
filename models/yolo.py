...
class IV6Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        #self.SP=SPreg(x)
        #self.SE = SEcls(x)
        self.cv2 = nn.ModuleList( # SPreg(x),
            nn.Sequential(SPreg(x),Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)
        self.cv3 = nn.ModuleList( # SEcls(x),
            nn.Sequential(SEcls(x),Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
        self.dfl = DFL(self.reg_max)
...
class SEcls(nn.Module):
    def __init__(self, channels, reduction=16):
        super(SEcls, self).__init__()
        self.fc1 = nn.Conv2d(channels//4, channels//4, kernel_size=1, padding=0)
        self.fc2 = nn.Conv2d(channels//4, channels//4, kernel_size=1, padding=0)
        self.fc3 = nn.Conv2d(channels//4, channels//4, kernel_size=1, padding=0)
        self.fc4 = nn.Conv2d(channels//4, channels//4, kernel_size=1, padding=0)
        self.relu = nn.SiLU(inplace=True)
        self.sigmoid = nn.Sigmoid()
        self.conv1 = nn.Conv2d(2, 1, 7, padding=3, bias=False)

        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fC1 = nn.Conv2d(channels//4, channels // reduction, kernel_size=1, padding=0)
        self.relu = nn.SiLU(inplace=True)
        self.fC2 = nn.Conv2d(channels // reduction, channels//4, kernel_size=1, padding=0)
        self.sigmoid = nn.Sigmoid()
        self.softmax = nn.Softmax(dim=1)
    def forward(self, x):
        a,b,c,d=torch.split(x, x.shape[1]//4, dim=1)[0],torch.split(x,x.shape[1]//4, dim=1)[1],torch.split(x, x.shape[1]//4, dim=1)[2],torch.split(x, x.shape[1]//4, dim=1)[3]
        x=a+b+c+d
        out = self.relu(self.fC1(self.avg_pool(x)))
        out = self.softmax(self.sigmoid(self.fC2(out)))
        a1, b1, c1, d1=self.fc1(out),self.fc2(out),self.fc3(out),self.fc4(out)
        #out=a1 * a, b1 * b, c1 * c, d1 * d
        out=torch.cat((a1*a,b1*b,c1*c,d1*d),1)
        return out

class SPreg(nn.Module):
    def __init__(self, channels, reduction=16):
        super(SPreg, self).__init__()
        #self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.f0 = nn.Conv2d(channels//4, 1, kernel_size=1, padding=0)
        self.relu = nn.SiLU(inplace=True)
        self.sigmoid = nn.Sigmoid()
        self.conv1 = nn.Conv2d(2, 1, 7, padding=3, bias=False)

        self.fc1 = nn.Conv2d(3, 1, kernel_size=1, padding=0)
        self.fc2 = nn.Conv2d(3, 1, kernel_size=1, padding=0)
        self.fc3 = nn.Conv2d(3, 1, kernel_size=1, padding=0)
        self.fc4 = nn.Conv2d(3, 1, kernel_size=1, padding=0)
        self.relu = nn.SiLU(inplace=True)
        self.sigmoid = nn.Sigmoid()
        self.softmax = nn.Softmax(dim=1)
    def forward(self, x):
        ...
        #print('xsp',avgout.shape)
        out = self.relu(self.f0(x))
        out= torch.cat([avgout,maxout,out], dim=1)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {NonLocal,SpatialTransformer,PSAModule,C2f,Bottleneck_c2,Bottleneck_,C3_,SKConv,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='STMA-Net.yaml', help='model.yaml')
